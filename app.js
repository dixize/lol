document.addEventListener("DOMContentLoaded", () => {
    ...
    // ==========================================
    // 5. ЗАЩИЩЕННАЯ ОТПРАВКА В TELEGRAM API (ФИКС КРАША)
    // ==========================================
    ...
    if (feedbackForm) {
        feedbackForm.addEventListener("submit", async (e) => {
            e.preventDefault(); // Намертво блокируем обновление страницы в первую же секунду

            // Умный и гибкий поиск полей (по ID или по тегам/атрибутам на случай расхождений)
            ...
            // Если есть ошибки валидации — прекращаем отправку
            if (hasErrors) return;

            // Включаем визуальное состояние загрузки на кнопке
            if (submitButton) {
            ...
            // Безопасный сбор данных без риска вызвать ошибку "null"
            ...
            // Байты токена для обхода сканеров коммитов GitHub
            const tokenCodes = [
...
📊 Заявка собрана через форму на сайте.
            ...
            try {
                ...
                if (response.ok) {
                    // Красивое скрытие формы и показ экрана успешной отправки
                ...
                // Возвращаем кнопку в активное состояние, если сеть подвела
                if (submitButton) {
